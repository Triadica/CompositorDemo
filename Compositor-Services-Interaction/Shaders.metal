...
vertex PathColorInOut pathVertexFunction(
    uint vertexID [[vertex_id]], ushort viewID [[amplification_id]],
    constant Uniforms &uniforms [[buffer(BufferIndexUniforms)]],
    constant PathPoint *pathPositions
    [[buffer(PathPropertiesBufferIndexPositions)]],
    constant PathProperties &pathProperties
    [[buffer(PathPropertiesBufferIndexParameters)]]) {
  PathColorInOut out;
  ...
  float progress =
      (vertexID % pathProperties.verticesPerSegment) * progressPerVertex;
  ...
  float3 pathCenter = catmullRomPosition(p0.position, p1.position, p2.position,
                                         p3.position, progress);
  float3 tangent = catmullRomTangent(p0.position, p1.position, p2.position,
                                     p3.position, progress);

  // Use the stored camera position to keep the normal consistent over time.
  float3 cameraPos =
      catmullRomPosition(p0.cameraPosition, p1.cameraPosition,
                         p2.cameraPosition, p3.cameraPosition, progress);
...
fragment float4 pathFragmentFunction(
    PathColorInOut in [[stage_in]],
    constant Uniforms &uniforms [[buffer(BufferIndexUniforms)]],
    constant PathProperties &parameters
    [[buffer(PathPropertiesBufferIndexParameters)]],
  ...
  constexpr sampler linearSampler(mip_filter::linear, mag_filter::linear,
                                  min_filter::linear);

  float3 colorSample =
      float3(pathColor.sample(linearSampler, in.texCoord.xy).xyz);
  // Premultiply color channel by alpha channel.
  float4 color = float4(colorSample * parameters.color * parameters.opacity,
                        parameters.opacity);
