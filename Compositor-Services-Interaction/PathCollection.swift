...
private class ActivePath: Path, Hashable, Equatable {
    ...
    func createCompletedPath() -> CompletePath {
        let bufferCopy = buffer.device.makeBuffer(
...
class PathCollection {
    ...
    init(layerRenderer: LayerRenderer) throws {
        ...
        pathColorTexture = try Self.loadTexture(
            device: layerRenderer.device, textureName: "PathColor")
        renderPipelineState = try Self.makePipelineDescriptor(layerRenderer: layerRenderer)
    }

    private class func loadTexture(
        device: MTLDevice,
        textureName: String
    ) throws -> MTLTexture {
...
extension PathCollection {
    ...
    private func returnPathStorageIfPossible(path: ActivePath) {
        ...
        if (path.didFinishEditing && path.renderingFrames.isEmpty) || path.parameters.opacity <= 0.0
        {
...
/// Path rendering.
extension PathCollection {
    func update(withTiming timing: LayerRenderer.Frame.Timing, worldTracking: WorldTrackingProvider)
        throws
    {
        let time = LayerRenderer.Clock.Instant.epoch.duration(to: timing.presentationTime)
            .timeInterval
        let cameraPosition =
            worldTracking.queryDeviceAnchor(atTimestamp: time)?.originFromAnchorTransform.columns.3
            .xyz ?? .zero
    ...
    func drawCommand(frame: LayerRenderer.Frame) throws -> PathCollectionDrawCommand {
        ...
        return PathCollectionDrawCommand(
            paths: renderedPaths,
    ...
    @RendererActor
    func encodeDraw(
        _ drawCommand: PathCollectionDrawCommand, encoder: MTLRenderCommandEncoder,
        ...
        for pathCommand in drawCommand.pathCommands {
            ...
            encoder.setVertexBytes(
                &parameters, length: MemoryLayout<PathProperties>.uniformStride,
            ...
            encoder.drawPrimitives(
                type: .triangleStrip, vertexStart: 0, vertexCount: pathCommand.vertexCount)
        }
        ...
        commandBuffer.addCompletedHandler { _ in
            Task { @MainActor [weak self] in
                ...
                for (_, path) in self.activePaths
                where path.renderingFrames.contains(drawCommand.frameIndex) {
    ...
    func updateUniformBuffers(
        _ drawCommand: PathCollectionDrawCommand,
...
struct PathCollectionDrawCommand {
    ...
    fileprivate init(paths: [Path], frameIndex: LayerFrameIndex, uniforms: MTLBuffer) {
        self.pathCommands = paths.map {
            .init(
                activePath: $0 as? ActivePath,
                buffer: $0.buffer,
