...
private let ceilingIndexesPerLamp: Int = patelPerLamp * 3  // cover the top of the block with triangles
// prepare the vertices for the block, 1 extra vertex for the top center of the block
private let indexesPerBlock = rectIndexesPerRect + ceilingIndexesPerLamp
// prepare the indices for the block
private let indexesCount: Int = lampCount * indexesPerBlock
...
class BlocksRenderer: CustomRenderer {
  ...
  private func createBlocksIndexBuffer(device: MTLDevice) {
    ...
    for i in 0..<lampCount {
      ...
      let indexBase = i * ceilingIndexesPerLamp
