...
kernel void spreadAroundBallComputeShader(
  ...
  if (leading) {
    ...
    // Add angular velocity component (rotation around Y axis)
    float3 radialDirection = normalize(cell.position - center);
    float3 angularAxis = float3(0.0, 1.0, 0.0);
    float3 tangentialDirection = normalize(cross(angularAxis, radialDirection));
    float angularForce = 0.002;
    
    // Add random drift to prevent direct center targeting
    float randomSeed = fract(sin(dot(cell.position.xy, float2(12.9898, 78.233))) * 43758.5453);
    float3 randomDrift = float3(
      sin(params.time * 0.5 + randomSeed * 6.28) * 0.001,
      cos(params.time * 0.3 + randomSeed * 4.71) * 0.001,
      sin(params.time * 0.7 + randomSeed * 3.14) * 0.001
    );
    
    // Combine forces: slight attraction to center, angular motion, random drift, outward expansion
    float3 centerAttraction = normalize(center - cell.position) * 0.0008;
    float3 outwardExpansion = normalize(cell.position - center) * 0.0003;
    float3 angularMotion = tangentialDirection * angularForce;
    
    float3 totalForce = centerAttraction + angularMotion + randomDrift + outwardExpansion;
    
    if (distanceToCenter <= r) {
      // Inside sphere: apply forces and gentle damping
      outputCell.velocity = cell.velocity * decay + totalForce * dt;
      ...
      if (info.intersected &&
        ...
        // Convert perpendicular velocity to tangential motion along sphere surface
        float3 tangentialFromPerp = normalize(cross(cross(info.normal, cell.velocity), info.normal));
        float perpSpeed = length(perpVelocity);
        
        // New velocity: keep parallel component, convert perpendicular to tangential
        float3 newVelocity = parallelVelocity * 0.9 + tangentialFromPerp * perpSpeed * 0.7 + tangentialDirection * 0.015;
