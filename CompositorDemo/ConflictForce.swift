...
@MainActor
class ConflictForceRenderer: CustomRenderer {
  ...
  init(layerRenderer: LayerRenderer) throws {
    ...
    let attractorUpdateBase = library.makeFunction(name: "conflictForceComputeShader")!
    ...
    for i in 0..<linesCount {
      ...
      for j in 0..<controlCountPerLine {
        ...
        attractorBase[index] = BounceBase(
          position: (p * 1 + SIMD3<Float>(0, 3, -1)),
          color: color,
          velocity: SIMD3<Float>(0, -0.0, 0) + p * 0.0)
      }
    }
    ...
    -> MTLRenderPipelineState
  {
    ...
    let vertexFunction = library.makeFunction(name: "conflictForceVertexShader")
    let fragmentFunction = library.makeFunction(name: "conflictForceFragmentShader")
