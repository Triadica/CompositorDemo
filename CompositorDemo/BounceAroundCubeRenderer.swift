...
@MainActor
class BounceAroundCubeRenderer: CustomRenderer {
  ...
  init(layerRenderer: LayerRenderer) throws {
    ...
    let attractorUpdateBase = library.makeFunction(name: "bounceAroundCubeComputeShader")!
    ...
    for i in 0..<linesCount {
      ...
      for j in 0..<controlCountPerLine {
        ...
        attractorBase[index] = BounceBase(
          position: p * 0.6 + SIMD3<Float>(0, 2, -1), color: color,
          velocity: SIMD3<Float>(0, 0.04, 0) + p * 0.04)
      }
    }
    ...
    -> MTLRenderPipelineState
  {
    ...
    let vertexFunction = library.makeFunction(name: "bounceAroundCubeVertexShader")
    let fragmentFunction = library.makeFunction(name: "bounceAroundCubeFragmentShader")
