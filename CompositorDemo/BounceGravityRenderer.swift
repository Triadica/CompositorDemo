...
@MainActor
class BounceGravityRenderer: CustomRenderer {
  ...
  init(layerRenderer: LayerRenderer) throws {
    ...
    let attractorUpdateBase = library.makeFunction(name: "bounceGravityComputeShader")!
    ...
    for i in 0..<linesCount {
      ...
      for j in 0..<controlCountPerLine {
        ...
        attractorBase[index] = BounceBase(
          position: (p * 0.3 + SIMD3<Float>(-2, 0, -1)), color: color,
          velocity: SIMD3<Float>(0, 0, 0) + p * 0.008)
      }
    }
    ...
    -> MTLRenderPipelineState
  {
    ...
    let vertexFunction = library.makeFunction(name: "bounceGravityVertexShader")
    let fragmentFunction = library.makeFunction(name: "bounceGravityFragmentShader")
